...
export const NOTIFICATION_KINDS = [
  ...
  "commented",
  "overdue_digest",
  // Weekly Goals (Manan 2026-06) — planner cron notifications. The `notify()`
  // dispatch matrix does not fan these out; the weekly-goals cron sends email
  // directly and inserts these in-app rows for the /inbox bell.
...
/**
 * Weekly Goals (Manan 2026-06) — fast-entry weekly priority planner that lives
 * next to Tasks. One row = one priority a team member commits to inside a
 * single Mon→Sun week. Client + Subject are free-text mirroring the same WMS
 * rosters used by tasks. `priority` reuses the Eisenhower enum so labels +
 * colours match Tasks. % done is employee-entered + admin-overridable; the
 * carry-over chain (`carriedFromId`) copies a goal forward without mutating
 * the original. See migration 0046_weekly_goals.sql.
 */
export const weeklyGoals = pgTable(
  "weekly_goals",
  {
    ...
    employeeId: uuid("employee_id")
      .notNull()
      .references(() => employees.id, { onDelete: "cascade" }),
    // Monday (IST) of the ISO week this goal lives in.
    weekStart: date("week_start").notNull(),
    // "Sr. No." — 1-based intent order within (employee, week).
    ...
    // "Target Done" — free-text definition of done for the week.
    ...
    // Carry-over chain: NULL on originals, set on a forward copy.
